    ...
    <aside className={`fixed left-0 top-0 h-screen bg-sidebar border-r border-sidebar-border flex flex-col z-30 transition-all duration-200 ${collapsed ? 'w-16' : 'w-60'}`}>
      ...
      <div className="flex items-center gap-3 px-4 h-16 border-b border-sidebar-border shrink-0">
        <button onClick={onToggle} className="flex items-center gap-2 w-full">
          <div className="w-8 h-8 rounded-lg bg-primary flex items-center justify-center font-display font-bold text-primary-foreground text-sm">N</div>
          {!collapsed && <span className="font-display font-bold text-foreground text-sm">NEXUS OS</span>}
        </button>
        {!collapsed && (
      ...
      {!collapsed && (
        <div className="mx-3 mb-3 p-3 rounded-lg bg-secondary/50 border border-border text-[11px]">
          <div className="text-muted-foreground uppercase tracking-wider mb-2 text-[10px]">Edge Status</div>
          <div className="space-y-1">
            ...
            <div className="flex justify-between"><span className="text-muted-foreground">CPU</span><span className="font-mono text-foreground">{edge.cpuPercent}%</span></div>
